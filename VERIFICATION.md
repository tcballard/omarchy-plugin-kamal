...
- Updated the installed plugin and confirmed the compact bar was better. The exact installed SHA was not captured.
...
Remaining before tagging: capture Omarchy/Quickshell and installed plugin revisions; validate the final candidate; verify remove/reinstall on the live desktop; merge release preparation with green CI and record the immutable release SHA. Terminal staging, real deploy/SSH, actual theme switching, multi-monitor and live vertical layout remain unverified and must not be presented as tested.
