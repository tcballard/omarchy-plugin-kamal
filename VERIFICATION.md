...
Remaining before tagging: capture the Quickshell version; validate the final candidate; verify remove/reinstall on the live desktop; merge release preparation with green CI and record the immutable release SHA. Terminal staging, real deploy/SSH, actual theme switching, multi-monitor and live vertical layout remain unverified and must not be presented as tested.
